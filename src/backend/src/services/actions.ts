...
import type { ActionResult, SlingRequest, RigAddRequest, CrewAddRequest } from "../types/gasown.js";
...
export async function addCrew(
  ...
  const result = await runGt(["crew", "add", request.name, "--rig", request.rig], {
    cwd: townRoot,
  });
...
export async function nudgeAgent(
  ...
  const result = await runGt(["nudge", agentAddress, message], { cwd: townRoot });
...
export async function runDoctor(
  ...
  return {
    ...
    message: result.exitCode === 0 ? "Health check passed" : "Health check found issues",
    data: {
