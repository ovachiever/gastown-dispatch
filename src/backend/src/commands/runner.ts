...
export async function runGt(
	...
	// Default to town root if no cwd specified
	if (!options.cwd) {
		const townRoot = findTownRoot();
		if (townRoot) {
			options = { ...options, cwd: townRoot };
		}
	}
...
export async function runGtJson<T>(
	...
	try {
		...
		throw new Error(
			`Failed to parse JSON from gt ${args.join(" ")}: ${result.stdout}`,
		);
	}
}
...
export async function runBdJson<T>(
	...
	try {
		...
		throw new Error(
			`Failed to parse JSON from bd ${args.join(" ")}: ${result.stdout}`,
		);
	}
}
